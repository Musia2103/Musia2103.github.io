...
var account = 0; // Текущий результат
var problem_timer; // Таймер для бомбочек
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        account.innerHTML = score;
    }
    else if (unit_classes.includes('problem-unit')) {
        finishTheGame();
        var account = document.getElementById("account");
        account.innerHTML = 'К сожалению, Вы попались на бомбочке';
    }
    return check;
...
function createProblem() {
    ...
    while (!problemCreated) { //пока бомбочки не создали
        // рандом
